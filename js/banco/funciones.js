$(document).ready(function(){
    listar();
    });


$("#formBanco").on('submit', function(e){
              e.preventDefault();        
              $.ajax({
                  type: 'POST',
                  url: '../ajaxphp/banco/guardar.php',
                  data: new FormData(this),
                  dataType: 'json',
                  contentType: false,
                  cache: false,
                  processData:false,
                beforeSend: function() {
                },
                success: function(response){
                    document.getElementById('formBanco').reset();
                    Swal.fire(
                          'Solicitud!',
                          response.message,
                          'success'
                        )

                                
                },
                error: function(error){
                    Swal.fire(
                              'Error!',
                              'Contacta al proveedor, no se pudo realizar su operación.',
                              'error'
                            )

                    
                  }
              })
    });

$("#updateformBanco").on('submit', function(e){
              e.preventDefault();        
              $.ajax({
                  type: 'POST',
                  url: '../ajaxphp/banco/actualizar.php',
                  data: new FormData(this),
                  dataType: 'json',
                  contentType: false,
                  cache: false,
                  processData:false,
                beforeSend: function() {
                },
                success: function(response){
                     Swal.fire(
                          'Solicitud!',
                          response.message,
                          'success'
                        )
                                
                },
                error: function(error){
                   Swal.fire(
                              'Error!',
                              'Contacta al proveedor, no se pudo realizar su operación.',
                              'error'
                            )

                    
                  }
              })
    });

function listar(){
         var estado = "1";
        jQuery.post("../ajaxphp/banco/listar.php", { 
        estado: estado },
        function(data, textStatus){
            $('#mostrarDatos').html(data);
        })
        }

function eliminar(id){
            var id_banco = id;
             Swal.fire({
              title: 'Estadas seguro?',
              text: "Estas seguro de eliminar el banco!",
              icon: 'warning',
              showCancelButton: true,
              confirmButtonColor: '#3085d6',
              cancelButtonColor: '#d33',
              confirmButtonText: 'Si'
            }).then((result) => {
              if (result.isConfirmed) {
                  $.ajax({
               type: 'POST',
               url: '../ajaxphp/banco/eliminar.php',
               data: { id_banco: id_banco },
                success: function(response){
   
                 Swal.fire(
                  'Solicitud!',
                  'El banco fue eliminado correctamente.',
                  'success'
                        )
                listar();
                 
              
                } ,
                error: function(error){
                   Swal.fire(
                              'Error!',
                              'Contacta al proveedor, no se pudo realizar su operación.',
                              'error'
                            )

                    
                  }


              })
                
            }
        }) 
    }
